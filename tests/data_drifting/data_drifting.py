...
def create_Dataloaders():
    ...
    data_loader_train = torch.utils.data.DataLoader(images_list_train, batch_size=2, shuffle=True, num_workers=4,
                collate_fn=utils.collate_fn)
    
    data_loader_test = torch.utils.data.DataLoader(images_list_test, batch_size=2, shuffle=True, num_workers=4,
                collate_fn=utils.collate_fn)
...
def instantiate_DD():
    feature_extractor = torchvision.models.detection.fasterrcnn_resnet50_fpn(pretrained=True)
    # num_classes which is user-defined
    num_classes = 2  # 1 class (person) + background
    in_features = feature_extractor.roi_heads.box_predictor.cls_score.in_features
    # replace the pre-trained head with a new one
    feature_extractor.roi_heads.box_predictor = FastRCNNPredictor(in_features, num_classes)
    # can be added later
    feature_extractor.load_state_dict(torch.load('../../models/sheep_train_augmented.pth', map_location=torch.device('cpu')))
...
# =============================================================================
    ...
    plt.scatter(0.5*np.random.randn(1, len(features)),features.numpy(), color='r', label='Benign Data')
    plt.scatter(0.5*np.random.randn(1,len(features_blurred)), features_blurred.numpy(), color='b', label='Corrupted Data')
