...
def createBinImgFromBB(image, annotation):
    ...
    binImg[
        int(bb_coords[1].item()) : int(bb_coords[3].item()),
        int(bb_coords[0].item()) : int(bb_coords[2].item()),
    ] = 1
...
def retrieveBBfromBinImg(image):
    ...
    x1 = np.nonzero(colsum)[0][0]
    x2 = np.nonzero(colsum)[0][-1] + 1
    y1 = np.nonzero(rowsum)[0][0]
    y2 = np.nonzero(rowsum)[0][-1] + 1
