...
def main():
    ...
    kaggle.api.dataset_download_files(
        "andrewmvd/sheep-detection", path=input_filepath, unzip=True
    )
    ...
    annotation_list_extension, images_list_extension = augmentDataset(
        annotation_list, images_list, "extension"
    )

    # split the extension-data into testing and training set

    (
        images_train_extension,
        images_test,
        annotations_train_extension,
        annotations_test,
    ) = train_test_split(
        images_list_extension,
        annotation_list_extension,
        test_size=0.33,
        random_state=42,
    )
    ...
    os.makedirs(test_filepath, exist_ok=True), os.makedirs(
        train_filepath, exist_ok=True
    )
    ...
    torch.save(
        images_list_augmented, "../../data/processed/train/images_list_augmented.pt"
    )
